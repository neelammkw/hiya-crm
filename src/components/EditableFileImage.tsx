...
interface Props {
  ...
  placeholder?: string;
  value?: string;
  defaultImage?: string; // New prop for the default image
  width?: number;
...
const EditableFileImage: FC<Props> = ({
  ...
  const isDefaultImage = value === defaultImage; // Check if the current image is the default image
  const marks = {
    100: '100px',
    150: '150px',
    200: '200px',
  ...
  const handleChangeWidth = (value: number) => {
    if (typeof onChangeWidth === 'function' && !isDefaultImage) {
      onChangeWidth(value);
    }
  };
  ...
  const clearImage = () => {
    if (typeof onChangeImage === 'function' && !isDefaultImage) {
      onChangeImage('');
    }
  };
